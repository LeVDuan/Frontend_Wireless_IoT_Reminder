...
const RenameDrawer = ({ open, toggle, device }: RenameDrawerProps) => {
  // ** Hooks
  ...
  const defaultValues = {
    currentName: device ? device.name : '',
    newName: ''
  }
  const {
    reset,
      ...
      >
        <Header>
          <Typography variant='h6'>
            Rename Device: {device.name}(#{device.deviceId})
          </Typography>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <FormControl fullWidth sx={{ mb: 6 }}>
              <Controller
                ...
                rules={{ required: true }}
                render={({ field: {} }) => (
                  <TextField value={device.name} label='Current Name' InputProps={{ readOnly: true }} />
                )}
              />
            </FormControl>
            <FormControl fullWidth sx={{ mb: 6 }}>
